...
fn main() {
    ...
    //
    if user_input_loop(&driver_manager).is_none() {
        return;
    };
...
fn user_input_loop(
    ...
    loop {
        ...
        match selection {
            ...
            2 => {
                // install driver
                driver_manager.install_driver(driver_manager);
            },
            3 => {
                // uninstall
                driver_manager.uninstall_driver(driver_manager);
            },
            4 => {
                // start driver
                driver_manager.start_driver(driver_manager);
            },
            5 => {
                // stop the driver
                driver_manager.stop_driver(driver_manager);
            },
